...
// Préparer la requête avec paramètre
$sql = "
    SELECT e.* 
    FROM evenement e
...
$en_cours = array_filter($evenements, function($e) use ($today) {
    return $e['date_debut'] <= $today && $e['date_fin'] >= $today;
});
$passes = array_filter($evenements, function($e) use ($today) {
    return $e['date_fin'] < $today;
});
$futurs = array_filter($evenements, function($e) use ($today) {
